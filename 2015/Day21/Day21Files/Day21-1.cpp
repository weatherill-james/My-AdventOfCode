...
bool fightWin(map<string, int> player, map<string, int> enemy) {
    bool returnBool;
...
int main() {
        ...
        for (auto tempArmor : shopArmor) {
            for (int tempRing1Num = 0; tempRing1Num < shopRings.size(); tempRing1Num += 1) {
                for (int tempRing2Num = tempRing1Num + 1; tempRing2Num < shopRings.size(); tempRing2Num += 1) {
                    ...
                    if (fightWin(playerStats, bossStats)) {
                        if (cost < lowestCost) {
                        	lowestCost = cost;
                        };
                    };
    ...
    cout << endl << "The cost of the lowest winning setup is: " << lowestCost << endl;
