...
int main() {
    ...
    int lowestNumberOfContainers = 10;
    int validCombinations = 0;
    for (string binString : binaryNums) {
        int tempSum = 0;
        int tempNumberOfContainers = 0;

        for (int i = 0; i < binString.size(); i += 1) {
            if (binString[i] == '1') {
                tempSum += containerSizes[i];
                tempNumberOfContainers += 1;
            };
        };

        if (tempSum == goal) {
            if (tempNumberOfContainers == lowestNumberOfContainers) {
                validCombinations += 1;
            } else if (tempNumberOfContainers < lowestNumberOfContainers) {
                validCombinations = 1;
                lowestNumberOfContainers = tempNumberOfContainers;
            };
        };
    };
