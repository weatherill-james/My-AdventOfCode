...
int main() {
    ...
    vector<vector<int>> incorrectlyOrderedChains;
    ...
    for (vector<int> chain : chainsOfPages) {
        ...
        if (incorrectlyOrdered) {
            incorrectlyOrderedChains.push_back(chain);
        };
    };

    vector<vector<int>> correctlyOrderedIncorrectChains;

    for (vector<int> chain : incorrectlyOrderedChains) {
        vector<int> tempChain = chain;
        while (true) {
            bool correctlyOrdered = true;

            for (int indexInTempChain = 0; indexInTempChain < tempChain.size() - 1; indexInTempChain += 1) {
                int keyNum = tempChain[indexInTempChain];
                int toFindNum = tempChain[indexInTempChain + 1];

                if (not pageBeforePages[keyNum].contains(toFindNum)) {
                    correctlyOrdered = false;
                    iter_swap(tempChain.begin() + indexInTempChain, tempChain.begin() + indexInTempChain + 1);
                    break;
                };
            };

            if (correctlyOrdered) {
                correctlyOrderedIncorrectChains.push_back(tempChain);
                break;
            };
        };
    };

    int middleNumberTotal = 0;

    for (vector<int> chain : correctlyOrderedIncorrectChains) {
