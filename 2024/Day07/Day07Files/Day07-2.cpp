...
string ternaryFinder(int num, int sizeOfString) {
    string ternaryString = "";

    while (num > 0) {
        ternaryString = to_string(num % 3) + ternaryString;
        num /= 3;
    };

    while (ternaryString.size() < sizeOfString) {
...
vector<int> inputsFinder(string inputString) {
    ...
    string tempNum = "";
    for (int i = 0; i < inputsSubstring.size(); i += 1) {
...
bool testInputOnOperators(string goalNum, vector<int> inputs) {
    for (int i = 0; i < 3 * pow(3, inputs.size() - 2); i += 1) {
        string ternaryString = ternaryFinder(i, inputs.size() - 1);

        long long tempAns = inputs[0];

        int index = 1;
        for (char chr : ternaryString) {
            if (chr == '0') {
                tempAns += inputs[index];
            } else if (chr == '1') {
                tempAns *= inputs[index];
            } else {
                tempAns = stoll(to_string(tempAns) + to_string(inputs[index]));
            };
